  ...
  */
  ...
  function update(){
   ...
   if(keyPressed.indexOf(39) != -1){
    ...
    tank.x = Math.min(screen.width - tankSprite.wdith - 30, tank.x); //Restricting the right side
    ...
    for(let i = 0; i < bullets.length; i++){
      ...
      if(bullet.y + bullet.height < 0 || bullet.y > screen.height){
        ...
        i--;//Redo current index since the array is shifted and current slot is replaced by the next bullet
        continue;
      }
